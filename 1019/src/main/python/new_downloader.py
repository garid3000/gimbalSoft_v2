...
#PnbgVEzCFguL$4E, TPV5M9C6g4JKDdF
#pypi-AgENdGVzdC5weXBpLm9yZwIkNDEyZDk1MmMtNDdhNC00NGE0LTk3MDktNDNhYmJmYmU1YjhjAAIleyJwZXJtaXNzaW9ucyI6ICJ1c2VyIiwgInZlcnNpb24iOiAxfQAABiC-L0A1Lhai4g23elKF_xqWklyWFy_d0U5kpxFvr8hgcg
#token
class downloader(object):
	...
	def download_1shot(self, each_image_id):
		...
		for iii in range(5):
			...
			try:
				...
				im = Image.open(fName)
				tmpy = np.array(im)
				im = Image.fromarray(tmpy.transpose())
				im.save(fName)
				print('saved')
			except:
	...
	def download_1shotBMP(self, each_image_id):
		tmp = 'http://spectrumcatcher.polarstarspace.com/veggie/report/view_report.php?id='+self.username+'&v=103&c='+self.pass_hex+'&fname=VeggieCamera_crops_spectrum_' # 20191213_140027_0.bmp'
		tmp = tmp + each_image_id + '_0.bmp'
		print(tmp)
		urllib.request.urlretrieve(tmp,  "tmp.tmp")
	...
	def download_all(self):
		for i in range(self.numShots):
			...
			self.download_1shot(self.ID_Shots[i])
	def download_allBMP(self):
		for i in range(self.numShots):
			print("======== " + self.date_str + '\t' + str(i) + " ========")
			self.download_1shotBMP(self.ID_Shots[i])
